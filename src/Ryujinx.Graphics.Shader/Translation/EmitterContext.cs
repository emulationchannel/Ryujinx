    ...
    class EmitterContext
    {
        ...
        public void PrepareForVertexReturn()
        {
            if (Config.GpuAccessor.QueryViewportTransformDisable())
            {
                ...
                Operand xScale = this.Load(StorageKind.ConstantBuffer, 0, Const((int)SupportBufferField.ViewportInverse), Const(0));
                Operand yScale = this.Load(StorageKind.ConstantBuffer, 0, Const((int)SupportBufferField.ViewportInverse), Const(1));
            ...
            else if (Config.Stage == ShaderStage.Fragment)
            {
                ...
                for (int rtIndex = 0; rtIndex < 8; rtIndex++)
                {
                    for (int component = 0; component < 4; component++)
                    {
                        ...
                        {
                            Operand isBgra = this.Load(StorageKind.ConstantBuffer, 0, Const((int)SupportBufferField.FragmentIsBgra), Const(rtIndex));
